...
// UIkit.util.on('#switcher', 'shown', function () {
...
function navControl(item){
    ...
    jQuery('#tab_8_prev').attr('uk-switcher-item', 'previous');
    break;
    case 'hideComm':
    jQuery('#commercialItem').hide();
    jQuery('#commercialNext').attr('uk-switcher-item','8');
    jQuery('#tab_8_prev').attr('uk-switcher-item','6')
    break;
    case 'showCommLess':
    ...
    jQuery('#tab_8_prev').attr('uk-switcher-item','5')
     ...
     jQuery('#switcher-nav li:nth-child(2)').addClass('uk-active');
                 ...
                 if(jQuery('.uk-radio[name="wasyour"]:checked').val() == null || jQuery('.uk-radio[name="wasyour"]:checked').val() == ''){
                   ...
                   return false;
                    ...
                    // 	return true;
                    if((jQuery('#less').val() == jQuery('#lessValid').val()) && lessThenYear == true) {
                      jQuery('#lessYear').hide();
               ...
               if(jQuery('.uk-radio[name="wasyourcomm"]:checked').val() == null || jQuery('.uk-radio[name="wasyourcomm"]:checked').val() == ''){
                 ...
                 return false;
                    ...
                    //  return true;
                    if((jQuery('#lessComm').val() == jQuery('#lessValidComm').val()) && lessThenYear == true) {
            ...
            function submit() {
              ...
              data['email'] = userEmail_GLOBAL;
              writeUserData(data,userID_GLOBAL);
            }

            function writeUserData(data, userID){
              firebase.database().ref('user/' + userID + "/").push(data,function(error){
                if(error){
                  console.log(error);
                }
                else{
